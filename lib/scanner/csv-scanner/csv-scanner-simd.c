 ...
 *
 * The broadcast constants are spelled out inline as `_mm256_set1_epi8` —
 * with `static inline` and AVX2 enabled the compiler folds these into
 * RIP-relative loads from .rodata and hoists them out of the chunk loop
 * in `csv_simd_parse`, so they cost nothing per call. */
AVX2_FN
static inline void
...
/* Inner state-machine step for csv_simd_parse.
 * Pulled out so the same code services both the full-chunk and tail paths
 * without code duplication, and lets the compiler keep the tight inner
 * vars in registers. */
AVX2_FN
...
/* Parse CSV format using SIMD classification.
 * Handles comma-delimited, double-quote quoted RFC4180 format only.
 * Populates fields_array with CSVFieldOfs for each field found.
 * `len` is the number of bytes in `input` (excluding any trailing NUL). */
AVX2_FN
gboolean
csv_simd_parse(const gchar *input, gsize len, GArray *fields_array)
{
  ...
  /* Phase 1: full 64-byte chunks read directly from input — no branch in
   * the inner loop deciding which buffer to read. */
  ...
  /* Phase 2: tail (< 64 bytes), via a stack-local bounce buffer. */
 ...
 *   - one inner function services find_either / find_either3 / find_either4
 * --------------------------------------------------------------------- */
 ...
 * On hit, fill in `result` and return TRUE.  On miss, return FALSE. */
AVX2_FN
static inline gboolean
  ...
  while (end_ofs - start_ofs >= 32)
    {
      ...
      if (mask != 0xFFFFFFFFu)
        return start_ofs + __builtin_ctz(~mask);
      start_ofs += 32;
    }
      ...
      if (mask != 0xFFFFFFFFu)
        {
          ...
          return chunk_start + last_non_ws + 1;
        }
      end_ofs = chunk_start;
    }
